...
export async function GET() {
  ...
  if (database) try { const row = await database.prepare('SELECT revision, data, updated_at FROM system_map_state WHERE id = 1').first<StoredRow>(); if (row) return json({ data: redactMapData(JSON.parse(row.data), access), revision: row.revision, updatedAt: row.updated_at, mode: 'shared' }); return json({ data: null, revision: 0, updatedAt: null, mode: 'shared' }); } catch { /* Fresh local previews can run before the migration. */ }
  return json({ data: redactMapData(memoryState?.data ?? null, access), revision: memoryState?.revision ?? 0, updatedAt: memoryState?.updatedAt ?? null, mode: 'memory' });
}
...
export async function PUT(request: Request) {
  ...
  if (database) try {
    ...
    if (current && current.revision !== expectedRevision) return json({ data: redactMapData(JSON.parse(current.data), access), revision: current.revision, updatedAt: current.updated_at, mode: 'shared' }, 409);
    const nextRevision = (current?.revision ?? 0) + 1;
    if (current) { const result = await database.prepare('UPDATE system_map_state SET revision = ?, data = ?, updated_at = ? WHERE id = 1 AND revision = ?').bind(nextRevision, serialized, updatedAt, expectedRevision).run(); if (!result.meta?.changes) { const latest = await database.prepare('SELECT revision, data, updated_at FROM system_map_state WHERE id = 1').first<StoredRow>(); return json({ data: redactMapData(latest ? JSON.parse(latest.data) : null, access), revision: latest?.revision ?? 0, updatedAt: latest?.updated_at ?? null, mode: 'shared' }, 409); } }
  ...
  if (memoryState && memoryState.revision !== expectedRevision) return json({ data: redactMapData(memoryState.data, access), revision: memoryState.revision, updatedAt: memoryState.updatedAt, mode: 'memory' }, 409);
