...
function UnifiedFlowMap({ modules, flows, selectedId, layout, onLayoutChange, onSelect, onAddRoot }: { modules: SystemModule[]; flows: DataFlow[]; selectedId: string; layout: FlowLayout; onLayoutChange: (layout: FlowLayout) => void; onSelect: (module: SystemModule) => void; onAddRoot: () => void }) {
  ...
  const measurePaths = useCallback(() => {
    ...
    setPaths(visibleFlows.flatMap((flow) => { const source = endpointRefs.current.get(flow.sourceId)?.getBoundingClientRect(); const target = endpointRefs.current.get(flow.targetId)?.getBoundingClientRect(); if (!source || !target) return [];
      ...
      if (horizontal) { const forward = dx >= 0; startX = (forward ? source.right : source.left) - bounds.left; endX = (forward ? target.left : target.right) - bounds.left; startY = source.top + source.height / 2 - bounds.top; endY = target.top + target.height / 2 - bounds.top; const bend = Math.max(42, Math.abs(endX - startX) * .45); d = `M ${startX} ${startY} C ${startX + (forward ? bend : -bend)} ${startY}, ${endX + (forward ? -bend : bend)} ${endY}, ${endX} ${endY}`; }
    ...
    <div className="workspace">
      <section className="map-panel" aria-label="系統模組視覺化"><div className="map-toolbar"><div className="search-box"><Search aria-hidden="true" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜尋模組或暫存實例…" aria-label="搜尋系統狀態" /></div><div className="filter-set" aria-label="模組健康狀態篩選">{(['all', 'healthy', 'watch', 'risk'] as const).map((filter) => <button key={filter} type="button" className={healthFilter === filter ? 'active' : ''} onClick={() => setHealthFilter(filter)}>{filter === 'all' ? '全部' : healthMeta[filter].label}{filter !== 'all' ? <b>{healthCounts[filter]}</b> : null}</button>)}</div></div>
        <div className="system-canvas"><div className="canvas-header"><div><span className="canvas-kicker">LIVE ARCHITECTURE / {version.label}</span><h2>模組、資料流與暫存狀態</h2></div><span className="canvas-hint"><CircleDot />點擊模組編輯狀態</span></div>
          <UnifiedFlowMap modules={visibleModules} flows={version.flows ?? []} selectedId={selectedModule?.id ?? ''} layout={flowLayout} onLayoutChange={chooseFlowLayout} onAddRoot={addRootModule} onSelect={(target) => { setSelectedModuleId(target.id); setSelectedTopicId(target.topics[0]?.id ?? null); setFlowPeerId(''); }} />
