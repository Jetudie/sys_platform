...
type TempSpace = { id: string; name: string; kind: string; detail: string; quantity?: number };
...
function versionHtml(version: SystemVersion, systemName: string) {
  ...
  const renderModule = (module: SystemModule, depth = 0): string => `<article style="margin-left:${depth * 22}px"><h2>${escape(module.name)} <small>${escape(module.code)}</small></h2><p>${escape(module.summary)}</p><h3>暫存空間</h3><ul>${module.spaces.flatMap((space) => expandedSpaceNames(space).map((name) => `<li><b>${escape(name)}</b> · ${escape(space.kind)} — ${escape(space.detail)}</li>`)).join('') || '<li>無</li>'}</ul><h3>主題與決議</h3>${module.topics.map((topic) => `<section><h4>${escape(topic.title)}</h4><span class="tag">${escape(topicMeta[topic.status].label)}</span><p>${escape(topic.content)}</p><small>${escape(topic.updatedAt)}</small></section>`).join('') || '<p>目前沒有主題。</p>'}${(module.children ?? []).map((child) => renderModule(child, depth + 1)).join('')}</article>`;
...
function ModuleBranch({ module, index, depth = 0, selectedId, onSelect }: { module: SystemModule; index: number; depth?: number; selectedId: string; onSelect: (module: SystemModule) => void }) {
  ...
  return <div className={`module-stack depth-${depth}`} style={{ '--module-color': module.color } as React.CSSProperties}>
    <button type="button" className={`${depth === 0 ? 'module-card' : 'submodule-card'} ${selectedId === module.id ? 'selected' : ''}`} onClick={() => onSelect(module)}>
      {depth === 0 ? <>
        <span className="module-index">{String(index + 1).padStart(2, '0')}</span><span className={`health-mark ${healthMeta[module.health].tone}`}>{healthMeta[module.health].label}</span><span className="module-title"><small>{module.code}</small><strong>{module.name}</strong></span><span className="module-summary">{module.summary}</span><span className="cache-rack">{module.spaces.flatMap((space) => expandedSpaceNames(space).map((name, instanceIndex) => <span className="cache-unit" key={`${space.id}-${instanceIndex}`}><Archive /><span><b>{name}</b><small>{space.kind}</small></span></span>))}</span><span className="module-footer"><span>{module.topics.length} 則主題</span>{children.length ? <span className="child-count"><Layers3 />{children.length} 子模組</span> : null}{discussions ? <span className="discussion-count"><MessageCircle />{discussions} 討論中</span> : <span className="review-clear"><Check />已整理</span>}<ChevronRight /></span>
      </> : <>
        <span className="submodule-line" aria-hidden="true" /><Layers3 /><span className="submodule-name"><small>{module.code}</small><strong>{module.name}</strong></span><span className="submodule-meta">{moduleSpaceCount(module)} 暫存區 · {module.topics.length} 主題</span><span className={`health-dot ${healthMeta[module.health].tone}`} title={healthMeta[module.health].label} /><ChevronRight />
      </>}
    </button>
...
export default function Home() {
  ...
  const addSpace = () => updateModule({ spaces: [...selectedModule.spaces, { id: makeId('space'), name: '新暫存空間', kind: 'Redis', detail: '補上容量、TTL 或用途。', quantity: 1 }] });
  ...
  return <main className="system-shell">
    ...
    <section className="version-strip" aria-label="系統版本">
      ...
      <div className="version-note"><Badge variant="outline">{version.release}</Badge><span>{version.description}</span></div><div className="version-metrics" aria-label="版本摘要"><span><strong>{allModules.length}</strong> 模組</span><span><strong>{allModules.reduce((sum, module) => sum + moduleSpaceCount(module), 0)}</strong> 暫存區</span><span><strong>{topicCounts.discussion}</strong> 討論中</span></div>
    </section>
    <div className="workspace">
      ...
      {selectedModule ? <aside className="detail-panel" aria-label={`${selectedModule.name} 詳細資訊`}>
        ...
        <Tabs defaultValue="topics" className="detail-tabs">
          ...
          <TabsContent value="spaces" className="tab-body"><div className="tab-actions"><p>設定基礎名稱與數量，畫布會自動展開成獨立空間。</p><Button size="sm" onClick={addSpace}><Plus />新增暫存區</Button></div><div className="space-list editable">{selectedModule.spaces.map((space, index) => <article key={space.id}><span className="space-no">{String(index + 1).padStart(2, '0')}</span><Archive /><div className="space-fields"><label><span>基礎名稱</span><Input value={space.name} onChange={(event) => updateSpace(space.id, { name: event.target.value })} /></label><label><span>類型</span><Input value={space.kind} onChange={(event) => updateSpace(space.id, { kind: event.target.value })} /></label><label className="space-quantity-field"><span>數量</span><Input type="number" min={1} step={1} inputMode="numeric" value={spaceQuantity(space)} onChange={(event) => updateSpace(space.id, { quantity: Math.max(1, Math.floor(Number(event.target.value)) || 1) })} /></label><label className="space-detail-field"><span>容量、TTL 或用途</span><Input value={space.detail} onChange={(event) => updateSpace(space.id, { detail: event.target.value })} /></label><div className="space-instance-preview"><span>展開顯示</span><div>{expandedSpaceNames(space).map((name, instanceIndex) => <code key={`${space.id}-preview-${instanceIndex}`}>{name}</code>)}</div></div></div><Button type="button" variant="ghost" size="icon-sm" className="space-delete" aria-label={`刪除 ${space.name}`} onClick={() => removeSpace(space.id)}><Trash2 /></Button></article>)}{!selectedModule.spaces.length ? <div className="detail-empty"><Archive /><span>這個模組尚無暫存空間</span><button type="button" onClick={addSpace}>加入第一個</button></div> : null}</div><p className="autosave-note"><Check />變更會同步到畫布與匯出文件</p></TabsContent>
