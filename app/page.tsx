    ...
    <div className="workspace">
      <section className="map-panel" aria-label="系統模組視覺化"><div className="map-toolbar"><div className="search-box"><Search aria-hidden="true" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜尋模組或暫存實例…" aria-label="搜尋系統狀態" /></div><div className="filter-set" aria-label="模組健康狀態篩選">{(['all', 'healthy', 'watch', 'risk'] as const).map((filter) => <button key={filter} type="button" className={healthFilter === filter ? 'active' : ''} onClick={() => setHealthFilter(filter)}>{filter === 'all' ? '全部' : healthMeta[filter].label}{filter !== 'all' ? <b>{healthCounts[filter]}</b> : null}</button>)}</div></div>
        <div className="system-canvas"><div className="canvas-header"><div><span className="canvas-kicker">LIVE ARCHITECTURE / {version.label}</span><h2>模組、資料流與暫存狀態</h2></div><span className="canvas-hint"><CircleDot />點擊模組編輯狀態</span></div>
          <div className="flow-map" aria-label="模組資料流"><div className="flow-map-title"><Waypoints /><span>資料流上下游</span><b>{(version.flows ?? []).length} 條</b></div><div className="flow-paths">{(version.flows ?? []).map((flow) => { const source = findModule(version.modules, flow.sourceId); const target = findModule(version.modules, flow.targetId); if (!source || !target) return null; return <div className="flow-path" key={flow.id}><button type="button" onClick={() => { setSelectedModuleId(source.id); setSelectedTopicId(source.topics[0]?.id ?? null); }}>{source.name}</button><span><ArrowRight />{flow.label || '資料流'}</span><button type="button" onClick={() => { setSelectedModuleId(target.id); setSelectedTopicId(target.topics[0]?.id ?? null); }}>{target.name}</button></div>; })}{!(version.flows ?? []).length ? <span className="flow-empty">尚未指定資料流，可在右側「資料流」頁籤新增。</span> : null}</div></div>
      ...
      {selectedModule ? <aside className="detail-panel" aria-label={`${selectedModule.name} 詳細資訊`}>
        ...
        <Tabs defaultValue="topics" className="detail-tabs">
          ...
          <TabsContent value="flows" className="tab-body"><div className="flow-editor-intro"><Waypoints /><div><strong>資料流上下游</strong><p>這裡描述模組間的資料傳遞；模組與子模組的包含關係仍由「結構」頁籤管理。</p></div></div><div className="flow-builder"><label><span>方向</span><Select value={flowDirection} onValueChange={(value) => value && setFlowDirection(value as 'upstream' | 'downstream')}><SelectTrigger aria-label="資料流方向"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="upstream">新增上游</SelectItem><SelectItem value="downstream">新增下游</SelectItem></SelectContent></Select></label><label><span>連接模組</span><Select value={flowPeerId} onValueChange={(value) => setFlowPeerId(value ?? '')}><SelectTrigger aria-label="選擇資料流模組"><SelectValue placeholder="選擇模組" /></SelectTrigger><SelectContent>{flowCandidates.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectContent></Select></label><Button type="button" onClick={addFlow} disabled={!flowPeerId}><Plus />新增關係</Button></div><div className="flow-editor-list">{selectedFlows.map((flow) => { const incoming = flow.targetId === selectedModule.id; const peerId = incoming ? flow.sourceId : flow.targetId; const peer = findModule(version.modules, peerId); return <article key={flow.id} className={incoming ? 'incoming' : 'outgoing'}><span className="flow-direction-icon">{incoming ? <ArrowLeft /> : <ArrowRight />}</span><div className="flow-peer"><small>{incoming ? '上游來源' : '下游目的'}</small><strong>{peer?.name ?? peerId}</strong></div><label htmlFor={`flow-label-${flow.id}`}><span>傳遞內容</span><Input id={`flow-label-${flow.id}`} value={flow.label} placeholder="例如：訂單事件" onChange={(event) => updateFlow(flow.id, { label: event.target.value })} /></label><Button type="button" variant="ghost" size="icon-sm" aria-label={`刪除與 ${peer?.name ?? peerId} 的資料流`} onClick={() => removeFlow(flow.id)}><Trash2 /></Button></article>; })}{!selectedFlows.length ? <div className="detail-empty"><Waypoints /><span>此模組尚未設定資料流</span></div> : null}</div></TabsContent>
