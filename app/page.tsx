    ...
    <div className="workspace">
      ...
      {selectedModule ? <aside className="detail-panel" aria-label={`${selectedModule.name} 詳細資訊`}>
        ...
        <Tabs defaultValue="topics" className="detail-tabs">
          ...
          <TabsContent value="spaces" className="tab-body"><p className="space-intro">每個小容器代表這一層模組擁有的暫存責任。</p><div className="space-list">{selectedModule.spaces.map((space, index) => <article key={space.id}><span className="space-no">{String(index + 1).padStart(2, '0')}</span><Archive /><div><strong>{space.name}</strong><span>{space.kind}</span><p>{space.detail}</p></div></article>)}</div></TabsContent>
