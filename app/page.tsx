...
function FlowNode({ module, role, onSelect }: { module: SystemModule; role: 'upstream' | 'focus' | 'downstream'; onSelect: (module: SystemModule) => void }) {
  const recordCount = moduleRecordCount(module);
  return <button type="button" className={`flow-node ${role}`} style={{ '--module-color': module.color } as React.CSSProperties} onClick={() => onSelect(module)} aria-label={`${module.name}，${role === 'upstream' ? '上游模組' : role === 'downstream' ? '下游模組' : '目前模組'}`}>
    <span className="flow-node-heading"><small>{module.code}</small><span className={`health-mark ${healthMeta[module.health].tone}`}>{healthMeta[module.health].label}</span></span>
    <strong>{module.name}</strong>
    <span className="flow-node-metrics"><span><Layers3 />{(module.children ?? []).length} 子模組</span><span><Database />{recordCount} 筆</span></span>
  </button>;
}

function FlowStage({ modules, flows, selectedModule, onSelect }: { modules: SystemModule[]; flows: DataFlow[]; selectedModule: SystemModule; onSelect: (module: SystemModule) => void }) {
  const incoming = flows.map((flow) => ({ flow, module: findModule(modules, flow.sourceId) })).filter((item): item is { flow: DataFlow; module: SystemModule } => item.flow.targetId === selectedModule.id && Boolean(item.module));
  const outgoing = flows.map((flow) => ({ flow, module: findModule(modules, flow.targetId) })).filter((item): item is { flow: DataFlow; module: SystemModule } => item.flow.sourceId === selectedModule.id && Boolean(item.module));
  return <section className="flow-stage" aria-label={`${selectedModule.name} 的上下游資料流`}>
    <div className="flow-stage-head"><div><span><Waypoints />上下游資料流</span><strong>目前聚焦：{selectedModule.name}</strong></div><p>點擊任一模組即可重新聚焦資料路徑</p></div>
    <div className="flow-tier-label"><ArrowUp /><span>上游輸入</span><b>{incoming.length}</b></div>
    <div className={`flow-tier upstream ${incoming.length ? '' : 'empty'}`}>
      {incoming.map(({ flow, module }) => <div className="flow-branch incoming" key={flow.id}><FlowNode module={module} role="upstream" onSelect={onSelect} /><span className="flow-connector"><i /><b>{flow.label || '資料流'}</b><ArrowDown /></span></div>)}
      {!incoming.length ? <span className="flow-stage-empty">沒有指定上游模組</span> : null}
    </div>
    <div className="flow-focus-wrap"><span className="flow-focus-line" aria-hidden="true" /><FlowNode module={selectedModule} role="focus" onSelect={onSelect} /><span className="flow-focus-line" aria-hidden="true" /></div>
    <div className={`flow-tier downstream ${outgoing.length ? '' : 'empty'}`}>
      {outgoing.map(({ flow, module }) => <div className="flow-branch outgoing" key={flow.id}><span className="flow-connector"><i /><b>{flow.label || '資料流'}</b><ArrowDown /></span><FlowNode module={module} role="downstream" onSelect={onSelect} /></div>)}
...
function ModuleBranch({ module, index, flows, depth = 0, selectedId, onSelect }: { module: SystemModule; index: number; flows: DataFlow[]; depth?: number; selectedId: string; onSelect: (module: SystemModule) => void }) {
  const children = module.children ?? [];
  const upstreamCount = flows.filter((flow) => flow.targetId === module.id).length;
  const downstreamCount = flows.filter((flow) => flow.sourceId === module.id).length;
  return <div className={`module-stack depth-${depth}`} style={{ '--module-color': module.color } as React.CSSProperties}>
    <button type="button" className={`${depth === 0 ? 'module-card' : 'submodule-card'} ${selectedId === module.id ? 'selected' : ''}`} onClick={() => onSelect(module)}>
      {depth === 0 ? <>
        <span className="module-status-row"><span className={`health-mark ${healthMeta[module.health].tone}`}>{healthMeta[module.health].label}</span><span className="module-index">{String(index + 1).padStart(2, '0')}</span></span><span className="module-title"><small>{module.code}</small><strong>{module.name}</strong></span><span className="cache-visual-rack">{module.spaces.flatMap((space) => expandedSpaceNames(space).map((name, instanceIndex) => <CacheVessel key={`${space.id}-${instanceIndex}`} name={name} count={instanceCount(space, instanceIndex)} />))}{!module.spaces.length ? <span className="no-cache-state"><Database />無暫存空間</span> : null}</span><span className="module-flow-state"><span><ArrowLeft />{upstreamCount} 上游</span><span><ArrowRight />{downstreamCount} 下游</span>{children.length ? <span><Layers3 />{children.length} 子模組</span> : null}<ChevronRight /></span>
      </> : <>
        <span className="submodule-line" aria-hidden="true" /><span className="submodule-status-row"><Layers3 /><span className="submodule-name"><small>{module.code}</small><strong>{module.name}</strong></span><span className={`health-mark ${healthMeta[module.health].tone}`}>{healthMeta[module.health].label}</span><ChevronRight /></span><span className="submodule-cache-rack">{module.spaces.flatMap((space) => expandedSpaceNames(space).map((name, instanceIndex) => <CacheVessel compact key={`${space.id}-${instanceIndex}`} name={name} count={instanceCount(space, instanceIndex)} />))}{!module.spaces.length ? <span className="no-cache-state compact"><Database />無暫存</span> : null}</span><span className="submodule-flow-state"><span>{upstreamCount} 上游</span><span>{downstreamCount} 下游</span></span>
      </>}
    </button>
    {children.length ? <div className="submodule-tree" aria-label={`${module.name} 的子模組`}>{children.map((child, childIndex) => <ModuleBranch key={child.id} module={child} index={childIndex} flows={flows} depth={depth + 1} selectedId={selectedId} onSelect={onSelect} />)}</div> : null}
  </div>;
}
...
export default function Home() {
  ...
  const addSpace = () => updateModule({ spaces: [...selectedModule.spaces, { id: makeId('space'), name: '新暫存空間', kind: 'Redis', detail: '補上容量、TTL 或用途。', quantity: 1, instanceCounts: [0] }] });
  ...
  const addFlow = () => {
    if (!flowPeerId || flowPeerId === selectedModule.id || !findModule(version.modules, flowPeerId)) return;
        ...
        <Tabs defaultValue="topics" className="detail-tabs">
          ...
          <TabsContent value="flows" className="tab-body">
            ...
            <div className="flow-builder">
              <label><span>方向</span><Select value={flowDirection} onValueChange={(value) => value && setFlowDirection(value as 'upstream' | 'downstream')}><SelectTrigger aria-label="資料流方向"><SelectValue /></SelectTrigger><SelectContent align="start" alignItemWithTrigger={false} sideOffset={8}><SelectItem value="upstream">新增上游</SelectItem><SelectItem value="downstream">新增下游</SelectItem></SelectContent></Select></label>
              <label><span>連接模組</span><Select value={flowPeerId} onValueChange={(value) => setFlowPeerId(value ?? '')}><SelectTrigger aria-label="選擇資料流模組"><SelectValue placeholder="選擇模組" /></SelectTrigger><SelectContent align="start" alignItemWithTrigger={false} sideOffset={8}>{flowCandidates.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectContent></Select></label>
              <Button type="button" onClick={addFlow} disabled={!flowPeerId}><Plus />新增關係</Button>
            </div>
            <div className="flow-editor-list">{selectedFlows.map((flow) => { const incoming = flow.targetId === selectedModule.id; const peerId = incoming ? flow.sourceId : flow.targetId; const peer = findModule(version.modules, peerId); return <article key={flow.id} className={incoming ? 'incoming' : 'outgoing'}><span className="flow-direction-icon">{incoming ? <ArrowLeft /> : <ArrowRight />}</span><div className="flow-peer"><small>{incoming ? '上游來源' : '下游目的'}</small><strong>{peer?.name ?? peerId}</strong></div><label htmlFor={`flow-label-${flow.id}`}><span>傳遞內容</span><Input id={`flow-label-${flow.id}`} value={flow.label} placeholder="例如：訂單事件" onChange={(event) => updateFlow(flow.id, { label: event.target.value })} /></label><Button type="button" variant="ghost" size="icon-sm" aria-label={`刪除與 ${peer?.name ?? peerId} 的資料流`} onClick={() => removeFlow(flow.id)}><Trash2 /></Button></article>; })}{!selectedFlows.length ? <div className="detail-empty"><Waypoints /><span>此模組尚未設定資料流</span></div> : null}</div>
          </TabsContent>
