...
一個輕量的 2D 系統視覺化與協作工具。可切換系統版本、以不限層數的父子模組描述架構、編輯各模組的暫存空間與溝通主題，並將目前版本匯出為 Markdown、HTML 或 JSON。
...
- 切換不同系統版本，立即顯示該版本的模組、暫存空間與主題。
- 點選模組後新增、修改主題內容，並標示為「草稿」、「討論中」或「已 Review」。
- 新增與編輯暫存空間的名稱、類型及補充資訊。
- 模組可遞迴包含子模組，資料結構不限制層數；介面可沿麵包屑逐層瀏覽。
- 多人共享同一份資料，不區分編輯者；頁面每 5 秒同步，並以 revision 避免靜默覆蓋較新的內容。
- 匯出目前版本的 Markdown、HTML 或 JSON 文件。
- 搜尋模組、暫存空間與主題，並依主題狀態篩選。
